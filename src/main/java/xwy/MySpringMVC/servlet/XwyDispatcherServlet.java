...
import xwy.MySpringMVC.context.XwyApplicationContext;
...
public class XwyDispatcherServlet extends HttpServlet {
	...
	public void init(ServletConfig config) throws ServletException {
		...
		System.out.println("XwySpring MVC is init.");
	}
	...
	private void initHandlerMappings(XwyApplicationContext context) {
		...
		for (Entry<String, Object> entry : ioc.entrySet()) {
			...
			if (!clazz.isAnnotationPresent(xwy.MySpringMVC.annotitain.XwyController.class)) {
				continue;
			}
	...
	public void applyDefaultViewName(HttpServletResponse resp, XwyModelAndView mv) throws Exception {
		if (null == mv) {
			return;
		}
	...
	private class HandlerAdapter {
		...
		public XwyModelAndView handle(HttpServletRequest req, HttpServletResponse resp, Handler handler)
				throws Exception {
			...
			boolean isModelAndView = handler.method.getReturnType() == XwyModelAndView.class;
			Object r = handler.method.invoke(handler.controller, paramValues);
			if (isModelAndView) {
				return (XwyModelAndView) r;
			} else {
				return null;
			}
	...
	private class ViewResolver {
		...
		protected String parse(XwyModelAndView mv) throws Exception {
